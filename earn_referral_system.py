...
def admin_save_settings():
    keys =['referral_reward', 'daily_bonus', 'min_withdraw', 'welcome_message', 'force_join', 'force_join_batch_size', 'admin_panel_url', 'bot_token']
    for k in keys:
...
async def check_force_join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if get_setting('force_join') == '1':
        ...
        if channels:
            # ✅ FIX: Agar user pehle verify ho chuka hai to dobara check mat karo
            # Isse /start karne pe baar baar channels nahi aayenge
            if user_id in FORCE_JOIN_VERIFIED_USERS:
                return
            ...
            # ✅ FIX: Sab channels join hain — user ko cache mein add karo
            # Ab yeh user /start kare to channels dobara nahi aayenge
            if not not_joined:
                FORCE_JOIN_VERIFIED_USERS.add(user_id)
                return

            # Kuch channels join nahi hue — buttons dikhao
            batch_size = get_setting('force_join_batch_size', int)
            if batch_size <= 0:
                batch_size = len(not_joined)
            current_batch = not_joined[:batch_size]
            remaining_after = len(not_joined) - len(current_batch)

            # --- 3 buttons per row: [CH1][CH2][CH3] / [CH4][CH5][CH6] ---
            row = []
            buttons = []
            ...
            msg = "🛑 *Please join the channel(s) below, then tap \"I've Joined\" to continue.*"
            if remaining_after > 0:
                msg += f"\n\n_There are {remaining_after} more channel(s) to join after this step._"
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_join":
        ...
        if not_joined:
            # Still not joined — show buttons again (check_force_join handles this, but fallback here)
            batch_size = get_setting('force_join_batch_size', int)
            if batch_size <= 0:
                batch_size = len(not_joined)
            current_batch = not_joined[:batch_size]
            ...
            msg = "❌ *Abhi bhi kuch channels join nahi kiye!*\n\n🛑 Neeche sabhi channels join karo, phir \"I've Joined\" dabao."
            if remaining_after > 0:
                msg += f"\n\n_{remaining_after} aur channel(s) bhi join karne honge baad mein._"
        ...
        # Ab yeh user /start kare to dobara channels nahi dikhenge
        FORCE_JOIN_VERIFIED_USERS.add(user_id)
        try:
            await query.message.delete()
        except Exception:
            pass
        await context.bot.send_message(chat_id=user_id, text="✅ *Thank you for joining!* You can now use the bot.", reply_markup=get_main_keyboard(user_id), parse_mode='Markdown')
        return
